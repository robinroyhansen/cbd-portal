...
const ALLOWED_PATH_PATTERN = /^articles\/[a-zA-Z0-9_-]+-\d+\.(jpg|jpeg|png|webp)$/;
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Sanitize articleId to prevent injection
    const sanitizedArticleId = articleId?.replace(/[^a-zA-Z0-9_-]/g, '') || 'temp';
    ...
    const fileName = `${sanitizedArticleId}-${timestamp}.${fileExt}`;
    ...
    // Update article with new image URL if articleId provided
    if (articleId && articleId !== 'temp') {
      ...
      if (updateError) {
        console.error('Error updating article image URL:', updateError);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Clear featured_image from article if articleId provided
    if (articleId) {
      await supabase
        .from('kb_articles')
        .update({
