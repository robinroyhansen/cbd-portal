...
import { BrandPublishWarningModal } from '@/components/admin/BrandPublishWarningModal';
...
export default function BrandReviewEditorPage() {
  ...
  // Brand publish warning modal
  const [showBrandPublishModal, setShowBrandPublishModal] = useState(false);
  const [pendingPublishAction, setPendingPublishAction] = useState<'review' | 'both' | null>(null);
  ...
  // Brand publish warning modal handlers
  const handlePublishWithBrandCheck = () => {
    // If brand is not published, show warning modal
    if (!brand?.is_published) {
      setShowBrandPublishModal(true);
      return;
    }
    // Brand is published, proceed with normal publish
    handlePublishReviewOnly();
  };

  const handlePublishReviewOnly = async () => {
    setShowBrandPublishModal(false);
  ...
  const handlePublishBoth = async () => {
    ...
    try {
      ...
      if (!brandRes.ok) {
        throw new Error('Failed to publish brand');
      }

      // Update local brand state
      setBrand(prev => prev ? { ...prev, is_published: true } : null);
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex justify-between items-start mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {/* Brand Publication Status Indicator */}
            <div className="mt-2">
              {brand?.is_published ? (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-green-100 text-green-800 text-sm rounded-full">
                ...
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-100 text-amber-800 text-sm rounded-full">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <PublicationStatus
                ...
                onPublish={handlePublishWithBrandCheck}
                onUnpublish={() => {
                  setFormData(prev => ({
                    ...prev,
                    is_published: false,
      ...
      {/* Brand Publish Warning Modal */}
      <BrandPublishWarningModal
        isOpen={showBrandPublishModal}
        brandName={brand?.name || ''}
        onCancel={() => setShowBrandPublishModal(false)}
        onPublishReviewOnly={handlePublishReviewOnly}
