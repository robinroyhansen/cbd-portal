...
// CONDITIONS and CONDITION_CATEGORIES are now imported from '../lib/research-conditions'
...
export function ResearchPageClient({ initialResearch, condition }: ResearchPageClientProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Mobile Filter Drawer - Bottom Sheet Design */}
      <MobileFilterDrawer
        ...
        filters={{
          ...
          yearRange,
          qualityRange,
          selectedCannabinoids: selectedCannabinoids as CannabinoidKeyType[],
        }}
        onFilterChange={(key, value) => {
          switch (key) {
            case 'selectedConditions':
              setSelectedConditions(value as ConditionKey[]);
              break;
            case 'selectedStudyTypes':
              setSelectedStudyTypes(value as StudyType[]);
              break;
            case 'selectedQualityTiers':
              setSelectedQualityTiers(value as QualityTier[]);
              break;
            case 'subjectType':
              setSubjectType(value as SubjectType);
              break;
            case 'yearRange':
              setYearRange(value as { min: number; max: number });
              break;
            case 'qualityRange':
              setQualityRange(value as { min: number; max: number });
              break;
            case 'selectedCannabinoids':
              setSelectedCannabinoids(value as CannabinoidKey[]);
              break;
          }
          setCurrentPage(1);
        }}
