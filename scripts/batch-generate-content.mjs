 ...
 *
 * Usage:
 *   node scripts/batch-generate-content.mjs
 *   node scripts/batch-generate-content.mjs --meta-only
 *   node scripts/batch-generate-content.mjs --country-only
 *   node scripts/batch-generate-content.mjs --batch-size=20
 */
...
const DELAY_BETWEEN_API_CALLS = 1500;
...
function detectCountry(text) {
  ...
  // Lower threshold to 1 match since many studies have limited data
  return maxScore >= 1 ? detectedCountry : null;
}
...
async function generateStudyContent(study, apiKey, maxRetries = 3) {
...
async function main() {
  ...
  if (!COUNTRY_ONLY && needsMeta && needsMeta > 0) {
    ...
    while (remaining > 0) {
      console.log(`\n🔄 Processing batch (${processed} done, ${remaining} remaining)...\n`);
      ...
      for (const study of studies) {
        ...
        try {
          const content = await generateStudyContent(study, anthropicKey);
  ...
  if (!META_ONLY && needsCountry && needsCountry > 0) {
    ...
    while (true) {
      const { data: studies, error: fetchError } = await supabase
        .from('kb_research_queue')
        .select('id, title, abstract, authors, publication')
        .eq('status', 'approved')
        .is('country', null)
        .order('relevance_score', { ascending: false, nullsFirst: false })
        .limit(BATCH_SIZE);
      ...
      console.log(`\n🔄 Processing batch (${studies.length} studies)...\n`);

      for (const study of studies) {
        // Combine all text fields for better detection
        const text = `${study.title || ''} ${study.abstract || ''} ${study.authors || ''} ${study.publication || ''}`;
