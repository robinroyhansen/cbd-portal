...
const SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ2cmRyeXZncWFyZmZnZHVqbWp6Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2NzkwMTc5NywiZXhwIjoyMDgzNDc3Nzk3fQ.SmqoTgGp3J6JEOuOiEl7IMd8whWz4qAvQABM7AUc7kY';
const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY || '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';
...
async function translateWithAnthropic(text, fieldName) {
  if (!text) return text;
...
async function translateArticle(article, index, total) {
  ...
  try {
    ...
    const title = await translateWithAnthropic(article.title, 'title');
    
    console.log('  → Translating content...');
    const content = await translateWithAnthropic(article.content, 'content');
    
    console.log('  → Translating excerpt...');
    const excerpt = await translateWithAnthropic(article.excerpt, 'excerpt');
    
    console.log('  → Translating meta_title...');
    const meta_title = await translateWithAnthropic(article.meta_title, 'meta_title');
    
    console.log('  → Translating meta_description...');
    const meta_description = await translateWithAnthropic(article.meta_description, 'meta_description');
...
async function main() {
  console.log('🇫🇮 Starting Finnish translation of 15 CBD Portal articles...\n');

  // Load articles from the JSON file created by fetch-untranslated.mjs
  const articlesJson = fs.readFileSync('/tmp/fi_batch_desc.json', 'utf8');
